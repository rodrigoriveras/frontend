...
function App() {
  ...
  const [error, setError] = React.useState('');
   ...
   async function signup(user = null){ // default user to null
    ...
    async function signuptab(user = null){ // default user to null
      TodoDataService.signuptab(user)
      .then(response =>{
      setToken(response.data.token);
      setUser(user.username);
      localStorage.setItem('token', response.data.token);
    ...
    <div className="App">
          ...
          <Link class="nav-link" to={"/table01"}>Table</Link>    
            ...
            { user ? (
              ...
              <>
               ...
               <Link class="nav-link" to={"/signuptab"}>Table</Link>            
              </>
           )}   
          </Container>  
          </Nav>
      ...
      <div className="container mt-4">
        <Switch>
          ...
          <Route path="/signuptab" render={(props)=>
          <Signuptab {...props} signuptab={signuptab} />
          }>
          </Route>          
